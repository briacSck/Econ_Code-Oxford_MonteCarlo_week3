"""
Paper 0018 -- Anti-Corruption Subsidies and R&D Efficiency
Simulation script: baseline replication + Monte Carlo missing-data analysis
Governing manual: RA_MISSING_DATA.pdf

USAGE:
  Phase 1 (baseline only):   python simulation_0018.py --mode baseline
  Phase 2 (smoke test):      python simulation_0018.py --mode smoke
  Phase 3 (full run):        python simulation_0018.py --mode full

Estimator: pyfixest.feols(), CRV1 clustered on firm; Two-way FE: | firm + year
"""
...
SOURCE_FILE = (
    ...
    / "source_artifacts/Paper_AntiCorruption/main_dataset.csv"
)
DATA_CSV = PAPER_DIR / "DATA.csv"
LOG_FILE = PAPER_DIR / "logs" / "simulation_0018.log"
...
class Config:
    ...
    REPORT_WORKBOOK = PAPER_DIR / "full_run" / "AntiCorruptionReport_0018.xlsx"
    SMOKE_WORKBOOK = PAPER_DIR / "smoke" / "SMOKE_AntiCorruptionReport_0018.xlsx"
    REGRESSION_TXT_DIR = PAPER_DIR / "regression_outputs"
    PROGRESS_LOG = PAPER_DIR / "logs" / "simulation_0018_progress.log"
...
def _print_baseline_table(fit, n_obs: int) -> None:
    sep = "=" * 72
    print(sep)
    print(f"BASELINE REGRESSION -- Table 5, Panel A  N={n_obs:,}")
    print(sep)
    tidy = fit.tidy()
...
# =============================================================================
def main():
    parser = argparse.ArgumentParser(description="Paper 0018 simulation")
    parser.add_argument("--mode", choices=["baseline", "smoke", "full"], default="baseline")
    parser.add_argument("--reload", action="store_true")
    args = parser.parse_args()

    Config.PROGRESS_LOG = PAPER_DIR / "logs" / f"simulation_0018_progress_{args.mode}.log"

    log.info("=" * 60)
    log.info(f"Paper 0018 -- Mode: {args.mode.upper()}")
    log.info("=" * 60)
