"""
Paper 0021 -- Reshaping Corporate Boards Through Mandatory Gender Diversity Disclosures
Simulation script: baseline replication + Monte Carlo missing-data analysis
Governing manual: RA_MISSING_DATA.pdf

USAGE:
  Phase 1 (baseline only):   python simulation_0021.py --mode baseline
  Phase 2 (smoke test):      python simulation_0021.py --mode smoke
  Phase 3 (full run):        python simulation_0021.py --mode full

Estimator: pyfixest.feols(), CRV1 clustered on gvkey; Two-way FE: | gvkey + fyear; DiD spec: n1pfdir1 ~ post1_x_treat1 + controls | gvkey + fyear; post1_x_treat1 = post1 * treat1 (interaction created in preprocessing); sizetausd logged in preprocessing
"""
...
SOURCE_FILE = (
    ...
    / "source_artifacts/Paper_CorporateBoards/dirfin_main.dta"
)
DATA_CSV = PAPER_DIR / "DATA.csv"
LOG_FILE = PAPER_DIR / "logs" / "simulation_0021.log"
...
class Config:
    ...
    REPORT_WORKBOOK = PAPER_DIR / "full_run" / "CorporateBoardsReport_0021.xlsx"
    SMOKE_WORKBOOK = PAPER_DIR / "smoke" / "SMOKE_CorporateBoardsReport_0021.xlsx"
    REGRESSION_TXT_DIR = PAPER_DIR / "regression_outputs"
    PROGRESS_LOG = PAPER_DIR / "logs" / "simulation_0021_progress.log"
...
def _print_baseline_table(fit, n_obs: int) -> None:
    sep = "=" * 72
    print(sep)
    print(f"BASELINE REGRESSION -- Table 3, Column 4  N={n_obs:,}")
    print(sep)
    tidy = fit.tidy()
...
# =============================================================================
def main():
    parser = argparse.ArgumentParser(description="Paper 0021 simulation")
    parser.add_argument("--mode", choices=["baseline", "smoke", "full"], default="baseline")
    parser.add_argument("--reload", action="store_true")
    args = parser.parse_args()

    Config.PROGRESS_LOG = PAPER_DIR / "logs" / f"simulation_0021_progress_{args.mode}.log"

    log.info("=" * 60)
    log.info(f"Paper 0021 -- Mode: {args.mode.upper()}")
    log.info("=" * 60)
