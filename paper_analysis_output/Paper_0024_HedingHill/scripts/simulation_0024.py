"""
Paper 0024 -- Hedging on the Hill: Political Risk and Idiosyncratic Volatility
Simulation script: baseline replication + Monte Carlo missing-data analysis
Governing manual: RA_MISSING_DATA.pdf

USAGE:
  Phase 1 (baseline only):   python simulation_0024.py --mode baseline
  Phase 2 (smoke test):      python simulation_0024.py --mode smoke
  Phase 3 (full run):        python simulation_0024.py --mode full

Estimator: pyfixest.feols(), CRV1 clustered on gvkey; Two-way FE: | gvkey + year
"""
...
SOURCE_FILE = (
    ...
    / "source_artifacts/Paper_HedingOnTheHill/firmcyclepanel.dta"
)
DATA_CSV = PAPER_DIR / "DATA.csv"
LOG_FILE = PAPER_DIR / "logs" / "simulation_0024.log"
...
class Config:
    ...
    REPORT_WORKBOOK = PAPER_DIR / "full_run" / "HedingHillReport_0024.xlsx"
    SMOKE_WORKBOOK = PAPER_DIR / "smoke" / "SMOKE_HedingHillReport_0024.xlsx"
    REGRESSION_TXT_DIR = PAPER_DIR / "regression_outputs"
    PROGRESS_LOG = PAPER_DIR / "logs" / "simulation_0024_progress.log"
...
def _print_baseline_table(fit, n_obs: int) -> None:
    sep = "=" * 72
    print(sep)
    print(f"BASELINE REGRESSION -- Table 2, Column 3  N={n_obs:,}")
    print(sep)
    tidy = fit.tidy()
...
# =============================================================================
def main():
    parser = argparse.ArgumentParser(description="Paper 0024 simulation")
    parser.add_argument("--mode", choices=["baseline", "smoke", "full"], default="baseline")
    parser.add_argument("--reload", action="store_true")
    args = parser.parse_args()

    Config.PROGRESS_LOG = PAPER_DIR / "logs" / f"simulation_0024_progress_{args.mode}.log"

    log.info("=" * 60)
    log.info(f"Paper 0024 -- Mode: {args.mode.upper()}")
    log.info("=" * 60)
