"""
Paper 0020 -- Competing for Digital Attention
Simulation script: baseline replication + Monte Carlo missing-data analysis
Governing manual: RA_MISSING_DATA.pdf

USAGE:
  Phase 1 (baseline only):   python simulation_0020.py --mode baseline
  Phase 2 (smoke test):      python simulation_0020.py --mode smoke
  Phase 3 (full run):        python simulation_0020.py --mode full

Estimator: pyfixest.feols(), CRV1 clustered on metaID; Two-way FE: | metaID + monthtime
"""
...
SOURCE_FILE = (
    ...
    / "source_artifacts/Paper_CompetingForAttention/SMJ_Final.dta"
)
DATA_CSV = PAPER_DIR / "DATA.csv"
LOG_FILE = PAPER_DIR / "logs" / "simulation_0020.log"
...
class Config:
    ...
    REPORT_WORKBOOK = PAPER_DIR / "full_run" / "CompetingAttentionReport_0020.xlsx"
    SMOKE_WORKBOOK = PAPER_DIR / "smoke" / "SMOKE_CompetingAttentionReport_0020.xlsx"
    REGRESSION_TXT_DIR = PAPER_DIR / "regression_outputs"
    PROGRESS_LOG = PAPER_DIR / "logs" / "simulation_0020_progress.log"
...
def _print_baseline_table(fit, n_obs: int) -> None:
    sep = "=" * 72
    print(sep)
    print(f"BASELINE REGRESSION -- Table 2, Column 4  N={n_obs:,}")
    print(sep)
    tidy = fit.tidy()
...
# =============================================================================
def main():
    parser = argparse.ArgumentParser(description="Paper 0020 simulation")
    parser.add_argument("--mode", choices=["baseline", "smoke", "full"], default="baseline")
    parser.add_argument("--reload", action="store_true")
    args = parser.parse_args()

    Config.PROGRESS_LOG = PAPER_DIR / "logs" / f"simulation_0020_progress_{args.mode}.log"

    log.info("=" * 60)
    log.info(f"Paper 0020 -- Mode: {args.mode.upper()}")
    log.info("=" * 60)
