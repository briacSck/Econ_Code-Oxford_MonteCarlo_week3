"""
Paper 0019 -- Anti-Discrimination Laws and Firm Performance
Simulation script: baseline replication + Monte Carlo missing-data analysis
Governing manual: RA_MISSING_DATA.pdf

USAGE:
  Phase 1 (baseline only):   python simulation_0019.py --mode baseline
  Phase 2 (smoke test):      python simulation_0019.py --mode smoke
  Phase 3 (full run):        python simulation_0019.py --mode full

Estimator: pyfixest.feols(), CRV1 clustered on cm_id; Two-way FE: | cm_id + fyear; state×year trend via i(state_id, fyear); Sample: census_region_south notna & randsamp1 notna
"""
...
SOURCE_FILE = (
    ...
    / "source_artifacts/Paper_AntidiscriminationLawsPerformance/ad_dataset_ms1.dta"
)
DATA_CSV = PAPER_DIR / "DATA.csv"
LOG_FILE = PAPER_DIR / "logs" / "simulation_0019.log"
...
class Config:
    ...
    REPORT_WORKBOOK = PAPER_DIR / "full_run" / "AntidiscrimLawsReport_0019.xlsx"
    SMOKE_WORKBOOK = PAPER_DIR / "smoke" / "SMOKE_AntidiscrimLawsReport_0019.xlsx"
    REGRESSION_TXT_DIR = PAPER_DIR / "regression_outputs"
    PROGRESS_LOG = PAPER_DIR / "logs" / "simulation_0019_progress.log"
...
def _print_baseline_table(fit, n_obs: int) -> None:
    sep = "=" * 72
    print(sep)
    print(f"BASELINE REGRESSION -- Table 4, Operating Profitability  N={n_obs:,}")
    print(sep)
    tidy = fit.tidy()
...
# =============================================================================
def main():
    parser = argparse.ArgumentParser(description="Paper 0019 simulation")
    parser.add_argument("--mode", choices=["baseline", "smoke", "full"], default="baseline")
    parser.add_argument("--reload", action="store_true")
    args = parser.parse_args()

    Config.PROGRESS_LOG = PAPER_DIR / "logs" / f"simulation_0019_progress_{args.mode}.log"

    log.info("=" * 60)
    log.info(f"Paper 0019 -- Mode: {args.mode.upper()}")
    log.info("=" * 60)
