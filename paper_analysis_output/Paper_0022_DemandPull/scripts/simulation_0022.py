"""
Paper 0022 -- Demand-Pull Innovation and Startup Revenue
Simulation script: baseline replication + Monte Carlo missing-data analysis
Governing manual: RA_MISSING_DATA.pdf

USAGE:
  Phase 1 (baseline only):   python simulation_0022.py --mode baseline
  Phase 2 (smoke test):      python simulation_0022.py --mode smoke
  Phase 3 (full run):        python simulation_0022.py --mode full

Estimator: pyfixest.feols(), robust SEs (hetero); Entity FE: | SN; DiD spec: RevenueLikert ~ Post + Post_x_Treatment + Age + WorkExp + EntrepExperience | SN; Post_x_Treatment = Post * Treatment (interaction created in preprocessing)
"""
...
SOURCE_FILE = (
    ...
    / "source_artifacts/Paper_DemandPull/STATA CODE 3560.dta"
)
DATA_CSV = PAPER_DIR / "DATA.csv"
LOG_FILE = PAPER_DIR / "logs" / "simulation_0022.log"
...
class Config:
    ...
    REPORT_WORKBOOK = PAPER_DIR / "full_run" / "DemandPullReport_0022.xlsx"
    SMOKE_WORKBOOK = PAPER_DIR / "smoke" / "SMOKE_DemandPullReport_0022.xlsx"
    REGRESSION_TXT_DIR = PAPER_DIR / "regression_outputs"
    PROGRESS_LOG = PAPER_DIR / "logs" / "simulation_0022_progress.log"
...
def _print_baseline_table(fit, n_obs: int) -> None:
    sep = "=" * 72
    print(sep)
    print(f"BASELINE REGRESSION -- Table 3, Panel A  N={n_obs:,}")
    print(sep)
    tidy = fit.tidy()
...
# =============================================================================
def main():
    parser = argparse.ArgumentParser(description="Paper 0022 simulation")
    parser.add_argument("--mode", choices=["baseline", "smoke", "full"], default="baseline")
    parser.add_argument("--reload", action="store_true")
    args = parser.parse_args()

    Config.PROGRESS_LOG = PAPER_DIR / "logs" / f"simulation_0022_progress_{args.mode}.log"

    log.info("=" * 60)
    log.info(f"Paper 0022 -- Mode: {args.mode.upper()}")
    log.info("=" * 60)
