"""
Paper 0023 -- Effect of IPO on Tax Avoidance
Simulation script: baseline replication + Monte Carlo missing-data analysis
Governing manual: RA_MISSING_DATA.pdf

USAGE:
  Phase 1 (baseline only):   python simulation_0023.py --mode baseline
  Phase 2 (smoke test):      python simulation_0023.py --mode smoke
  Phase 3 (full run):        python simulation_0023.py --mode full

Estimator: pyfixest.feols(), CRV1 clustered on gvkey; Year FE absorbed: | fyear
"""
...
SOURCE_FILE = (
    ...
    / "source_artifacts/Paper_EffectOfIPO/sample_full.xlsx"
)
DATA_CSV = PAPER_DIR / "DATA.csv"
LOG_FILE = PAPER_DIR / "logs" / "simulation_0023.log"
...
class Config:
    ...
    REPORT_WORKBOOK = PAPER_DIR / "full_run" / "EffectIPOReport_0023.xlsx"
    SMOKE_WORKBOOK = PAPER_DIR / "smoke" / "SMOKE_EffectIPOReport_0023.xlsx"
    REGRESSION_TXT_DIR = PAPER_DIR / "regression_outputs"
    PROGRESS_LOG = PAPER_DIR / "logs" / "simulation_0023_progress.log"
...
def _print_baseline_table(fit, n_obs: int) -> None:
    sep = "=" * 72
    print(sep)
    print(f"BASELINE REGRESSION -- Table 3, Column 1  N={n_obs:,}")
    print(sep)
    tidy = fit.tidy()
...
# =============================================================================
def main():
    parser = argparse.ArgumentParser(description="Paper 0023 simulation")
    parser.add_argument("--mode", choices=["baseline", "smoke", "full"], default="baseline")
    parser.add_argument("--reload", action="store_true")
    args = parser.parse_args()

    Config.PROGRESS_LOG = PAPER_DIR / "logs" / f"simulation_0023_progress_{args.mode}.log"

    log.info("=" * 60)
    log.info(f"Paper 0023 -- Mode: {args.mode.upper()}")
    log.info("=" * 60)
