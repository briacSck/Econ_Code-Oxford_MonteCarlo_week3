"""
Paper 0025 -- Pathways to Profits: Impact of Marketing vs. Finance Skills
Simulation script: baseline replication + Monte Carlo missing-data analysis
Governing manual: RA_MISSING_DATA.pdf

USAGE:
  Phase 1 (baseline only):   python simulation_0025.py --mode baseline
  Phase 2 (smoke test):      python simulation_0025.py --mode smoke
  Phase 3 (full run):        python simulation_0025.py --mode full

Estimator: pyfixest.feols(), robust SEs (hetero); No entity FE (RCT); Industry dummies absorbed via | Ind2_SIC (see extra_preprocessing); Sample: Sample_endline == 1; pre_Profits3_composite_w1 included as control for baseline outcome
"""
...
SOURCE_FILE = (
    ...
    / "source_artifacts/Paper_PathwaysToProfits/P2P_dataset.dta"
)
DATA_CSV = PAPER_DIR / "DATA.csv"
LOG_FILE = PAPER_DIR / "logs" / "simulation_0025.log"
...
class Config:
    ...
    CONTROLS = [
        ...
        "Race_Foreigner",
        "Educ_high",
        "Operating_yearstotal",
        "Activity_Hours",
        "pre_Employees1_composite",
        "FormalReg",
    ...
    PREDICTOR_POOL = [
        ...
        "Ind2_SIC76",
        "Ind2_SIC83",
        "Distance2training_miles",
    ]

    MECHANISMS = ["MCAR", "MAR", "NMAR"]
    METHODS = ["LD", "Mean", "Reg", "Iter", "RF", "DL", "MILGBM"]

    REPORT_WORKBOOK = PAPER_DIR / "full_run" / "PathwaysProfitsReport_0025.xlsx"
    SMOKE_WORKBOOK = PAPER_DIR / "smoke" / "SMOKE_PathwaysProfitsReport_0025.xlsx"
    REGRESSION_TXT_DIR = PAPER_DIR / "regression_outputs"
    PROGRESS_LOG = PAPER_DIR / "logs" / "simulation_0025_progress.log"
...
def _print_baseline_table(fit, n_obs: int) -> None:
    sep = "=" * 72
    print(sep)
    print(f"BASELINE REGRESSION -- Table 5, Column 6  N={n_obs:,}")
    print(sep)
    tidy = fit.tidy()
...
# =============================================================================
def main():
    parser = argparse.ArgumentParser(description="Paper 0025 simulation")
    parser.add_argument("--mode", choices=["baseline", "smoke", "full"], default="baseline")
    parser.add_argument("--reload", action="store_true")
    args = parser.parse_args()

    Config.PROGRESS_LOG = PAPER_DIR / "logs" / f"simulation_0025_progress_{args.mode}.log"

    log.info("=" * 60)
    log.info(f"Paper 0025 -- Mode: {args.mode.upper()}")
    log.info("=" * 60)
